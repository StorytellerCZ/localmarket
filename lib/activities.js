...
Meteor.methods({
  createActivity: function(activity, tweet, loc) {
    ...
    const id = Activities.insert(activity);
...
if (Meteor.isServer) {
  const twitterOauth = function(options) {
    const config = Meteor.settings.twitter
    const userConfig = Meteor.user().services.twitter;
  ...
  const tweetActivity = function(activity) {
    ...
    const image = activity.image.replace(/^data.*base64,/, '');
  ...
  const getLocationPlace = function(loc) {
    const url = 'https://api.twitter.com/1.1/geo/reverse_geocode.json'
    ...
    if (response.statusCode === 200 && response.data) {
      const place = response.data.result.places.find(function(place) {
